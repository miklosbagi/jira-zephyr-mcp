...
| **Test cases** | Get one, **search** (**`search_test_cases`**: `GET /testcases/nextgen` + optional client-side text filter; v0.18.1 — not `/testcases/search`), cursor list (**`list_test_cases_nextgen`**, v0.14.0), create, update, create multiple |
...
**Test case search (v0.18.1):** Scale Cloud has **no** Server-style **`GET /testcases/search`**. That path is routed as **`GET /testcases/{testCaseKey}`** with key `"search"` → HTTP **400**. **`search_test_cases`** now uses **`GET /testcases/nextgen`**; optional **`query`** filters name/key/objective client-side. For folder-scoped discovery: **`list_folders`** (`folderType: TEST_CASE`) → **`search_test_cases`** / **`list_test_cases_nextgen`** with **`folderId`**.
